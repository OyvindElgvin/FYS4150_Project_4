...
If a phase transition is observed in a finite system, the critical temperature, $T_{c}(L_i)$, is possible to derive. The value of $T_{c}(L=\infty)$ can be derived if one has two temperatures from finite systems by using \eqref{eq:tc}. This would result in two equations with two unknown ($T_c(L=\infty)$ and $a$):
\begin{align*}
    \begin{cases}
...
Table \ref{tab:configurations} list the different spin configurations with their respective degeneracy, energy and magnetization, as where Table \ref{tab:benchmarks} displays some calculations from the 2x2 case.
...
\begin{table}[H]
    ...
    \caption{List of analytical and experimental values for the $2x2$ case. All values are given per spin.}
